        ...
        <x-sidebar sticky stashable class="border-r border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900">
            ...
            <x-navlist>
                ...
                <x-navlist.group :heading="__('Control')">
                    <x-navlist.item before="phosphor-truck" :href="route('admin.control.index')" :current="request()->routeIs('admin.control.*')">
                        {{ __('Control') }}
                    </x-navlist.item>

                    <x-navlist.item before="phosphor-truck" :href="route('admin.damages.index')" :current="request()->routeIs('admin.damages.*')">
                        {{ __('Damages') }}
                        @php
                            $pendingDamages = \App\Models\DamageReport::where('status', 'reported')->count();
                        @endphp
                        @if($pendingDamages > 0)
                            <span class="ml-auto inline-block py-0.5 px-2 text-xs rounded-full bg-red-100 text-red-600">
                                {{ $pendingDamages }}
                            </span>
                        @endif
                    </x-navlist.item>   
